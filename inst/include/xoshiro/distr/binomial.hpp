#ifndef XOSHIRO_DISTR_BINOMIAL_HPP
#define XOSHIRO_DISTR_BINOMIAL_HPP
...
T binomial_inversion(rng_state_t<T>& rng_state, int n, T p) {
  T u = xoshiro::unif_rand<T>(rng_state);
...
inline double btrs(rng_state_t<T>& rng_state, double n, double p) {
  ...
  while (true) {
    double u = xoshiro::unif_rand<T, double>(rng_state);
    double v = xoshiro::unif_rand<T, double>(rng_state);
    u = u - 0.5;
