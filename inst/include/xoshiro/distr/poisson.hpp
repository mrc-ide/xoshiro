#ifndef XOSHIRO_DISTR_POISSON_HPP
#define XOSHIRO_DISTR_POISSON_HPP
...
int rpois(rng_state_t<real_t>& rng_state,
  ...
  if (lambda < 10) {
    ...
    while (true) {
      real_t u = xoshiro::unif_rand<real_t>(rng_state);
      prod = prod * u;
    ...
    // http://citeseer.ist.psu.edu/viewdoc/citations?doi=10.1.1.48.3054
    //
    ...
    while (true) {
      real_t u = xoshiro::unif_rand<real_t>(rng_state);
      u -= 0.5;
      real_t v = xoshiro::unif_rand<real_t>(rng_state);
