...
Ordinarily this interface is used from C++. However, an R interface is provided for debugging and testing purposes.

```{r}
rng <- xoshiro::rng$new(seed = 1)
rng
```
...
One feature that we use here is to allow multiple streams of random numbers within a rng object. If running in parallel (via C++ code) these different random number streams can be given to different threads.

```{r}
rng1 <- xoshiro::rng$new(seed = 1, n_generators = 1)
rng2 <- xoshiro::rng$new(seed = 1, n_generators = 2)
rng1$unif_rand(5)
rng2$unif_rand(10)
```
...
```{r}
rng3 <- xoshiro::rng$new(seed = 1, n_generators = 1)$jump()
rng3$unif_rand(5)
```
...
Supported distributions are documented more-fully in `?xoshiro::rng`.
