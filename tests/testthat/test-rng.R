...
test_that("can generate random numbers", {
  ans1 <- rng$new(1, 1)$unif_rand(100)
  ans2 <- rng$new(1, 1)$unif_rand(100)
  ans3 <- rng$new(2, 1)$unif_rand(100)
...
test_that("Create interleaved rng", {
  ...
  rng1 <- rng$new(seed, 1L)
  rng2 <- rng$new(seed, 2L)
  rng3 <- rng$new(seed, 4L)
  rng4 <- rng$new(seed, 8L)
...
test_that("run uniform random numbers", {
  ans1 <- rng$new(1L, 1L)$unif_rand(100)
  ans2 <- rng$new(1L, 1L)$unif_rand(100)
  ans3 <- rng$new(1L, 1L)$runif(100, 0, 1)
  ans4 <- rng$new(2L, 1L)$runif(100, 0, 1)
...
test_that("run uniform random numbers with odd bounds", {
  ans <- rng$new(1L, 1L)$runif(100, -100, 100)
...
test_that("run binomial random numbers", {
  ...
  ans1 <- rng$new(1, 1)$rbinom(m, n, p)
  ans2 <- rng$new(1, 1)$rbinom(m, n, p)
  expect_identical(ans1, ans2)
...
test_that("binomial numbers run the short circuit path", {
  ...
  expect_identical(rng$new(1, 1)$rbinom(m, 0L, p),
                   rep(0L, m))
  expect_identical(rng$new(1, 1)$rbinom(m, n, 0),
                   rep(0L, m))
  expect_identical(rng$new(1, 1)$rbinom(m, n, 1),
                   rep(n, m))
})
...
test_that("binomial numbers and their complement are the same (np small)", {
  ...
  ans1 <- rng$new(1, 1)$rbinom(m, n, p)
  ans2 <- rng$new(1, 1)$rbinom(m, n, 1 - p)
...
test_that("poisson numbers", {
  ...
  ans1 <- rng$new(1, 1)$rpois(n, lambda)
  ans2 <- rng$new(1, 1)$rpois(n, lambda)
  ans3 <- rng$new(2, 1)$rpois(n, lambda)
  expect_identical(ans1, ans2)
...
test_that("jump", {
  seed <- 1
  rng1a <- rng$new(seed, 1L)
  rng1b <- rng$new(seed, 1L)$jump()
  rng2 <- rng$new(seed, 2L)
...
test_that("long jump", {
  seed <- 1
  rng1 <- rng$new(seed, 1L)
  rng2 <- rng$new(seed, 1L)$jump()
  rng3 <- rng$new(seed, 1L)$long_jump()
  rng4 <- rng$new(seed, 1L)$long_jump()$jump()
...
test_that("get state", {
  seed <- 1
  rng1 <- rng$new(seed, 1L)
  rng2 <- rng$new(seed, 1L)
  rng3 <- rng$new(seed, 2L)
...
test_that("initialise parallel rng with binary state", {
  seed <- 42
  rng1 <- rng$new(seed, 5L)
  state <- rng1$state()
  rng2 <- rng$new(state, 5L)
  r1 <- rng1$unif_rand(10)
  r2 <- rng2$unif_rand(10)
  expect_identical(r1, r2)
  expect_identical(rng1$state(), rng2$state())
...
test_that("initialise with NULL, generating a seed from R", {
  set.seed(1)
  rng1 <- rng$new(NULL, 1L)
  set.seed(1)
  rng2 <- rng$new(NULL, 1L)
  rng3 <- rng$new(NULL, 1L)
...
test_that("negative seed values result in sensible state", {
  ...
  ## pathalogical)
  s0 <- rng$new(0, 1L)$state()
  s1 <- rng$new(-1, 1L)$state()
  s10 <- rng$new(-10, 1L)$state()
...
test_that("can jump the rng state with rng_state_long_jump", {
  rng <- rng$new(1)
  state <- rng$state()
  r1 <- rng$long_jump()$state()
  r2 <- rng$long_jump()$state()

  expect_identical(rng_state_long_jump(state), r1)
  expect_identical(rng_state_long_jump(state), r1)
  expect_identical(rng_state_long_jump(state, 2), r2)
})
